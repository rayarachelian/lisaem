...
SET PACKAGES=%PACKAGES%,mingw64-x86_64-binutils,mingw64-i686-gcc,mingw64-i686-gcc-core mingw64-i686-gcc-debug-info,mingw64-i686-gcc-g++,mingw64-i686-gcc-objc,gcc-core,gcc-g++,make
SET PACKAGES=%PACKAGES%,upx,netpbm,p7zip,xz,zip,unzip,lynx,curl,lz4,libpng,libpng-devel,giflib,openal,vim,openjpeg2

 
REM -- Do it!
ECHO *** INSTALLING DEFAULT PACKAGES
e:\setup-x86.exe --quiet-mode --download --local-install --no-verify -s %SITE% -l "%LOCALDIR%" -R "%ROOTDIR%"
ECHO.
ECHO.
ECHO *** INSTALLING CUSTOM PACKAGES
e:\setup-x86.exe -q -d -D -L -X -s %SITE% -l "%LOCALDIR%" -R "%ROOTDIR%" -P %PACKAGES%
