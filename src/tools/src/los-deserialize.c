...
void deserialize(DC42ImageType *F)
{
             ...
             mddfsec[0x19]=='m'    )
         {
             ...
             uint32 lisa_sn=                      (mddfsec[0xcd]<<16) | (mddfsec[0xce]<<8) | (mddfsec[0xcf]);
             ...
             switch (fsversion) {
                ...
                default:   printf("*** UNKNOWN FILE SYSTEM VERSION %02x (expected 0e,0f, or 11)***\n",fsversion);
             };
             ...
             if (disk_sn) {
                 ...
                 printf("Disk MDDF signed by Lisa 0x%08x (AppleNet 001%05d, LOS Lisa #%d) zeroed out.\n",disk_sn,lisa_sn,los_lisa_num);

/*
I'm glad you brought this up - I think I just figured it out, and it's a weird algorithm...
Since we can now generate any Lisa serial number we want and plug it into LisaEm, I tried a couple combinations and found this pattern:
...
*/
         ...
         for (int sec=28; (unsigned)sec<F->numblocks; sec++)
         {
               char name[64];
               ...
               if (ftag[4]==0xff)         // tool entry tags have tag 4 as ff
               {
                     ...
                     fsec[s-3]=='}' && tolower(fsec[s-2])=='o' && tolower(fsec[s-1])=='b' && tolower(fsec[s])=='j' && fsec[0x182]<64 )
                  {
                    ...
                    uint32 lisa_sn=(mddfsec[0x43]<<16) | (mddfsec[0x44]<<8) | (mddfsec[0x45]);
                    ...
                    if (  toolsn != 0 || (fsec[0x48] | fsec[0x49] ) )     
                       {
                        fprintf(stderr, 
                             "Deserialized/debozo-ized Office System tool %s serialized for Lisa 0x%02x%02x%02x%02x (AppleNet: 001%05d) bozo bits:%02x%02x\n",
                             name,
                             fsec[0x42], fsec[0x43], fsec[0x44], fsec[0x45], lisa_sn,
                             fsec[0x48], fsec[0x49]);
